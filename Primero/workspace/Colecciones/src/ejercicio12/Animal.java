...
public abstract class Animal implements Comunicarse, Cloneable, Comparable<Animal>{
	...
	Animal(String nombre, double cuotaMensual, String raza, Date fnac){
		this.nombre=nombre;
	...
	public String toString() {
		 ...
		 for(i=0; i<crias.length; i++)
			 if(crias[i]!=null)
				 cadena+="\n\t" + crias[i].toString();
		return cadena + String.format("%nCodigo: %s%nFnac:%s%nHablar: %s%nEnfadarse: %s", codigo, fnac.toString(), hablar(), enfadarse());
	}
	...
	public boolean equals(Object obj) {
		...
		if(obj instanceof Animal && nombre.equals(((Animal)obj).nombre)
				&& cuotaMensual==((Animal)obj).cuotaMensual && raza.equals(((Animal)obj).raza) && Arrays.equals(((Animal)obj).crias,crias) && codigo.equals(((Animal)obj).codigo) && fnac.equals(((Animal) obj).fnac))
				resultado=true;
		return resultado;		
	}
	...
	public int compareTo(Animal o) {
		int comp = fnac.compareTo(o.fnac);
		if (comp == 0)
