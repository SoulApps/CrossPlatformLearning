...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            result = Funciones.select(String.format("SELECT CodAsignatura FROM horario h, tramohorario t WHERE h.CodTramo = t.CodTramo AND CodCurso= '%s' AND CodOe = '%s' ORDER BY dia, t.CodTramo ASC, 1;", curso, oe));
            dia = Funciones.select("SELECT DISTINCT dia FROM tramohorario ORDER BY dia;");
            hora = Funciones.select("SELECT DISTINCT HoraInicio, HoraFin FROM tramohorario ORDER BY dia, 1, 2;");
